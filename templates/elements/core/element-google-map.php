...
.info {
	...
	bottom: 10%;
	width: 50%;
	background: rgba(217, 237, 247, 0.58);
	z-index: 10000;
}
...
<div class="row google-map">
	<section class="acf-map main">
		<?php
			...
			$showonload		= get_sub_field('show_overlay_on_page_load');

		?>
			<div class="marker location-number-<?=$i;?>" data-location-info-number="<?=$i;?>" data-marker-type="<?php echo get_sub_field('marker_type'); ?>" data-lat="<?php echo $location['lat']; ?>" data-lng="<?php echo $location['lng']; ?>" <?php if ( $marker_image !== '' ) { echo 'data-markerimage="'.$marker_image.'"'; } ?> <?php if ( $title !== '' ) { echo 'data-title="'.$title.'"'; } ?> <?php if ( $icon_overlay !== '' ) { echo 'data-labelicon="'.$icon_overlay.'"'; } ?> <?php if ( $make_icon_spin == 'fa-spin' ) { echo 'data-icon-spin="fa-spin"'; } ?> <?php if ( $showonload == 'true' ) { echo 'data-info-display="true"'; } ?> >
	...
	?>
			...
			<!--div class="info location-info-number-<?=$i;?> <?php if ( $showonload == 'true' ) { echo 'info-display'; } ?> alert alert-info alert-dismissible fade in">
...
jQuery(function($) {
	...
	*/
	...
	function render_map( $el ) {
		...
		var style_default = [];
		<?php } elseif ( get_sub_field('map_styles') == 'custom' ) { ?>
	...
	*/
	...
	function add_marker( $marker, map ) {
		...
		var locationnumber		=	$marker.data('location-info-number');
		var infodisplay			= 	$marker.data('data-info-display');
		...
		if( $marker.html() )
		{
			...
			google.maps.event.addListener(marker, 'click', function() {
				...
				$('.info').hide();										// Hide the info windows for all the others
				$('.location-info-number-'+locationnumber+'').show();	// Show the info window for this one though
